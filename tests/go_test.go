...
func CheckByKey(fail func(string, ...interface{})) {
	...
	fire := &example.AbilityT{Id: 1}
	water := &example.AbilityT{Id: 8}
	earth := &example.AbilityT{Id: 6}
	air := &example.AbilityT{Id: 2}

	abilityOffsets := make([]flatbuffers.UOffsetT, 4)
	abilityOffsets[0] = fire.Pack(b)
	abilityOffsets[1] = water.Pack(b)
	abilityOffsets[2] = earth.Pack(b)
	abilityOffsets[3] = air.Pack(b)
	testarrayofsortedstruct := b.CreateVectorOfSortedTables(abilityOffsets, example.AbilityKeyCompare)


	example.MonsterStart(b)
	example.MonsterAddName(b, name)
	example.MonsterAddTestarrayoftables(b, testarrayoftables)
	example.MonsterAddTestarrayofsortedstruct(b, testarrayofsortedstruct)
	...
	fireAbility := &example.Ability{}
	monster.TestarrayofsortedstructByKey(fireAbility, fire.Id)
	waterAbility := &example.Ability{}
	monster.TestarrayofsortedstructByKey(waterAbility, water.Id)
	airAbility := &example.Ability{}
	monster.TestarrayofsortedstructByKey(airAbility, air.Id)
